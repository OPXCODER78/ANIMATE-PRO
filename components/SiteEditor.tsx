...
const SiteEditor: React.FC<SiteEditorProps> = ({ initialSite, onSave, onCancel }) => {
  ...
  const getIframeContent = useCallback((site: GeneratedSite) => `
    <html>
      <head>
        <meta charset="UTF-8">
      ...
      <body ${site.javascript && site.html.includes('three-canvas') ? 'data-is-3d="true"' : ''}>
        ${site.html}
  ...
  useEffect(() => {
    ...
    const handleIframeLoad = () => {
      ...
      if (doc && doc.defaultView) {
        doc.querySelectorAll('script, style').forEach(el => el.setAttribute('contenteditable', 'false'));
        ...
        doc.body.addEventListener('click', (event) => {
          const targetElement = event.target as HTMLElement;
          setContextMenu(prev => ({ ...prev, visible: false })); // Hide context menu
    ...
    iframe.srcdoc = getIframeContent(initialSite);
  ...
  const addAssetToIframe = (assetWrapper: HTMLElement) => {
    ...
    if (iframeBody) {
        ...
        assetWrapper.style.left = `${scrollX + 50}px`; 
        assetWrapper.style.top = `${scrollY + 50}px`;
        assetWrapper.setAttribute('data-x', (scrollX + 50).toString());
        assetWrapper.setAttribute('data-y', (scrollY + 50).toString());
  ...
  const handleNewImageFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file && iframeRef.current?.contentDocument) {
      try {
        ...
        wrapperDiv.style.width = "250px"; wrapperDiv.style.height = "200px";
    ...
    if (file && iframeRef.current?.contentDocument) {
      ...
      try {
        ...
        if (svgElement && svgElement.nodeName.toLowerCase() === 'svg') {
          ...
          const wrapperDiv = iframeRef.current.contentDocument.createElement('div');
          wrapperDiv.className = 'editable-asset'; wrapperDiv.style.width = "120px"; 
          wrapperDiv.style.height = "120px"; wrapperDiv.style.overflow = "visible";
  ...
  const handleSave = () => {
    if (iframeRef.current && iframeRef.current.contentDocument) {
      ...
      doc.querySelectorAll('[data-editing]').forEach(el => el.removeAttribute('data-editing'));
  ...
  return (
    <div className="space-y-6 relative"> {/* Added relative for context menu positioning */}
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center">
        <div>
          <h2 className="text-2xl font-semibold text-transparent bg-clip-text bg-gradient-to-r from-yellow-400 to-orange-500 mb-1">
            Site Visual Editor ({initialSite.name})
          </h2>
      ...
      {isLoading && (<div className="flex justify-center items-center h-[70vh] bg-slate-700 rounded-md"><p className="text-slate-300 text-lg">Loading editor...</p></div>)}

      <div className={`p-1 bg-slate-700 rounded-md ${isLoading ? 'hidden': ''}`}>
        <iframe ref={iframeRef} key={initialSite.id + initialSite.html.substring(0,100)} title="Site Visual Editor" className="w-full h-[70vh] md:h-[80vh] border-none rounded-md bg-white" sandbox="allow-scripts allow-same-origin allow-forms allow-modals" />
      </div>
      ...
      <div className="flex justify-end space-x-4 mt-6">
        ...
        <button onClick={handleSave} disabled={isLoading} className="px-6 py-2 text-base font-medium rounded-md text-white bg-green-600 hover:bg-green-700 transition-colors disabled:opacity-50 flex items-center space-x-2" aria-label="Save HTML changes"><CheckIcon className="w-5 h-5" /><span>Save HTML</span></button>
      </div>
