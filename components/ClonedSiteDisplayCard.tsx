...
interface ClonedSiteDisplayCardProps {
  site: GeneratedSite;
  onStartEdit: () => void;
}
...
const ClonedSiteDisplayCard: React.FC<ClonedSiteDisplayCardProps> = ({ site, onStartEdit }) => {
  ...
  const renderContent = () => {
    const safeSiteNameBase = site.name.replace(/[^a-z0-9_]/gi, '_').toLowerCase() || 'cloned_site';
    switch (activeTab) {
      case Tab.Preview:
        return (
          <iframe
            key={`site-preview-${site.id}-${iframeKey}`}
            ref={iframeRef}
            title={`site-preview-${site.name}`}
            className="w-full h-96 md:h-[600px] border-none rounded-md bg-white"
      ...
      case Tab.CSS:
        return site.customCss ? (
          <CodeBlock language="css" code={site.customCss} filename={`${safeSiteNameBase}.css`} />
        ) : (
          <p className="text-slate-400 p-4 text-center">No custom CSS provided for this site structure.</p>
        );
      case Tab.JS:
        return site.javascript ? (
          <CodeBlock language="javascript" code={site.javascript} filename={`${safeSiteNameBase}.js`} />
        ) : (
          <p className="text-slate-400 p-4 text-center">No JavaScript provided for this site structure.</p>
        );
      default:
        return null;
    }
  ...
  const tabButtonClasses = (tabName: Tab) => 
    ...
    (activeTab === tabName 
      ? 'bg-slate-700 text-green-400 border-b-2 border-green-500' 
        ...
        {activeTab === Tab.Preview && (
          <div className="flex items-center space-x-2">
            <button
              ...
              aria-label="Open site preview in new tab"
            >
            ...
            <button
              ...
              aria-label="Edit site content in dedicated editor"
            >
              <PencilIcon className="w-4 h-4 sm:w-5 sm:h-5" />
            </button>
            <button
              onClick={handleRefreshPreview}
              className="p-1.5 text-slate-400 hover:text-green-400 transition-colors duration-150 rounded-md hover:bg-slate-700"
...
export default ClonedSiteDisplayCard;